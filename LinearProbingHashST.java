...
public class LinearProbingHashST<Key, Value> implements STInterface<Key, Value> {
    ...
    public LinearProbingHashST(int capacity, boolean resize) {
        M = capacity;
        autoresize = resize;
        keys = (Key[])   new Object[M];
        vals = (Value[]) new Object[M];
    ...
    private void resize(int capacity) {
        LinearProbingHashST<Key, Value> temp = new LinearProbingHashST<Key, Value>(capacity, false);
        for (int i = 0; i < M; i++) {
            if (keys[i] != null) {
    ...
    public void put(Key key, Value val) {
        ...
        //unless autoresizeing has been turned off, double table size if 50% full
        if (N >= M/2 && autoresize) 
        	resize(2*M);
    ...
    public void delete(Key key) {
        ...
        if (N > 0 && N <= M/8 && autoresize) resize(M/2);
